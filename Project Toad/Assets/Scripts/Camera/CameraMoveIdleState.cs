...
public class CameraMoveIdleState : CameraMoveState
{
    const float _viewportLengthPercent = 100.0f;
    const float _zoomOutThreshold = (0.1f * 0.1f) * 100.0f;
    const float _zoomInThreshold = (0.25f * 0.25f) * 100.0f;
    ...
    public override CameraMoveState onUpdate()
    {
        ...
        // Zoom out.
        if (playerViewportDiff < _zoomOutThreshold)
        {
            return CameraController.CamMoveZoomOutState;
        }
        // Zoom in.
        else if (playerViewportDiff > _zoomInThreshold) 
        {
            return CameraController.CamMoveZoomInState;
        }
